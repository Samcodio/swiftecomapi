...
namespace SwiftEcom.Services
{
    ...
    public class EcommerceService
    {
        ...
        public static class TenantContext
        {
            ...
            public const string StoreId = "queenofafricaempire";
        ...
        public ApiResponse<List<ProductDTO>> GetProductsByCategory(string categoryId)
        {
            try
            {
                ...
                var products = db.Products
                    .Where(p => subcategoryIDs.Contains(p.Category)
                    //&& p.Status != "Private" 
                    && p.ActiveStatus == "Active" && db.Stores.Any(s => s.ID == p.MyStore && s.ID == TenantContext.StoreId))
                    .ToList()
                    .Select(p => MapProductToDTO(p))
                    .ToList();
        ...
        public ApiResponse<List<ProductDTO>> GetProductsBySubcategory(string subcategoryId)
        {
            try
            {
                ...
                var products = db.Products
                    .Where(p => p.Category == subcategoryId
                    //&& p.Status != "Private" 
                    && p.ActiveStatus == "Active" && db.Stores.Any(s => s.ID == p.MyStore && s.ID == TenantContext.StoreId))
                    .ToList()
                    .Select(p => MapProductToDTO(p))
